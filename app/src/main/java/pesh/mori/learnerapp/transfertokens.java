...
public class transfertokens extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAlert2 = new AlertDialog.Builder(getActivity());
...
//        txtUserEmail = transfertokens.findViewById(R.id.txt_tokens_transfer_email);
//        txtUserEmail.setEnabled(false);
        ...
        btnTransfer.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                mProgress.setTitle("Sending Tokens");
                ...
                FirebaseDatabase.getInstance().getReference().child("Bio").child(userId).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        if (!dataSnapshot.exists() || dataSnapshot.child("about").getValue().equals("")){
                            txtAbout.setText("Bio information not available.");
                        } else {
                        ...
                        if (!avatar.isEmpty()){
                            ...
                            Picasso.with(getContext()).load(R.mipmap.ic_user).into(imgAvatar);
                        }
...
//        String email = txtUserEmail.getText().toString().trim();
        if (amount.isEmpty() && !userId.isEmpty()){
            mAlert.setTitle("Enter required info")
        ...
        if (userId.isEmpty() && !amount.isEmpty()){
            mAlert2.setTitle("Enter required info")
                    .setMessage("Select the recepient of your tokens from the list by tapping the button below")
                    .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialogInterface, int i) {
                    ...
                    .setNegativeButton("Select User from List", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialogInterface, int i) {
                            startActivityForResult((new Intent(getActivity(),SearchUserActivity.class)),GET_USER_REQUEST);
                        }
    ...
    public void createFinancialAccount(){
        ...
        FirebaseDatabase.getInstance().getReference().child("MonetaryAccount").child(mAuth.getCurrentUser().getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast.makeText(getActivity(), "An error occured: "+databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
                ...
                FirebaseDatabase.getInstance().getReference().child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        if (buyerBalance<price){
                            Toast.makeText(getActivity(), "Your balance is insufficient by "+(price-buyerBalance)+" tokens", Toast.LENGTH_SHORT).show();
                            mProgress.dismiss();
                        } else {
    ...
    public void getReceiverDetails(final String key, final Double bal, final Double amt){
        FirebaseDatabase.getInstance().getReference().child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                FirebaseDatabase.getInstance().getReference().child("MonetaryAccount").child(receiverId).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        try{
                            if (!dataSnapshot.exists()){
                                DatabaseReference mMonetaryAccount = FirebaseDatabase.getInstance().getReference().child("MonetaryAccount").child(receiverId);
